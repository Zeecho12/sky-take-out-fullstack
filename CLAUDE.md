...
- **Phase**:功能 0005「C 端重建④:订单管理」**Phase 3 执行中,前四步(步骤1+2+3+4)完成 TESTED,进详情页**。分支 `feature/0005-order-manage`(自 main):规划 `947ca28`、快照 `102fa31`、**步骤1 `353f772`**(归属越权 D1)、**步骤2 `a296f2e`**(退款口径 D2)、**步骤3 `8b5d584`**(脚手架:4 类型+5 api[historyOrders 用 `pageNum`]+3 路由+3 占位)、**步骤4 `7c961f9`**(历史订单页 List.vue:van-tabs 3 tab+van-list 无限滚动,含一处 van-list 切 tab 缺陷修复)。六决策拍板:D1 归属越权 4 处全修(Service 层)/ D2 退款口径三处统一置 REFUND / D3 历史订单 3 tab+van-list / D4 三页三路由+Menu「我的」入口 / D5 用户中心纯导航壳。验证:后端 verifier 8/8+4/4;步骤3 type-check+curl+preview;步骤4 preview mobile 全 PASS(tab 过滤各切 tab 恰 1 请求且自动加载/无限滚动首屏单请求+触底无重复+finished 停/按钮按状态/点卡片跳详情)。(0004 merge `b08bf8e`、0003 `3365f69`、0002 `df53f0b`、0001 `b02590b` 均已交付。)
- **进行中**:epic「C 端完整重建」—— **0001–0004 已交付,0005(收官)Phase 3 执行中(6 步:步骤1/2/3/4 done,余步骤5–6 前端页面)**。跨功能决策(Vant / 整站门槛 / 改造复用 / 认证复用 0001)已落地。
- **下一步**:**0005 Phase 3 步骤5(前端订单详情页 Detail.vue + 动作 + 成功页接线,D4)**:`Detail.vue`(占位→实现)`GET orderDetail/{route.params.id}` 全字段+明细;按状态出操作(取消 status1|2→showConfirmDialog→cancel / 立即支付 status1→/order-pay 带 orderNumber+amount+orderId / 催单 status2→reminder→toast / 再来一单 always→showConfirmDialog→repetition 合并→cart.refresh→/menu,AD1 Q1);**Created.vue 查看订单去 disabled+接线 /order-detail/${orderId}、缺 orderId 兜底(禁用/退化 /order-list)**;Confirm.vue/Pay.vue query 每跳补透传 orderId(源自 submit OrderSubmitVO.id)。复用 List.vue 动作写法。派 subagent 实现(铁律 8)、verifier 跑 preview mobile 门(详情字段/取消/支付/催单/再来一单合并/成功页接线闭环/orderId 兜底负例)。之后步骤6(Center+Menu入口)。环境:新 jar(:8080)+ dev server(user-web:5173)在跑;**验证前先 preview_resize mobile**(否则 van-list/滚动类不触发);登录注入捷径写 `sky_user_token`/`sky_user_info`;甲(id=8)现有 16 单(status1×3=13/14/15、status2×2=16/17、余 status6)。按铁律 1 先读 proposal 交接头复述后动手。
