...
- **下一步**:**0005 Phase 4(验证收尾 + 合并回 main)**:①端到端冒烟(0002-0005 主链一次跑通)+ 更新 `docs/smoke-tests.md`(如需);②合并 `feature/0005-order-manage` 回 `main`(铁律 2 一功能一次合并);③复核 ADR-0005 决策落地一致性 + 收口 divedeep backlog(ADR 判本功能无"高含金量必单开"链路,越权修复并入鉴权主题即可,Phase 4 与 Tech Lead 确认);④契约校准:`用户端接口.md` line64 / ADR AD1 HIGH#2 `page` 报错"400"→"400/500(缺参异常,均非 Result)"(verifier 实测 500);⑤再生派生文档(如里程碑需要);⑥更新 blueprint(epic 0002-0005 收官)+ 本快照。环境:新 jar(:8080,含步骤1+2)+ dev server(user-web:5173)+ sky-redis 在跑;验证前先 `preview_resize` mobile;登录注入捷径写 `sky_user_token`/`sky_user_info`;账号 `s7v_2268`/`123456`(id=8)、admin `admin`/`123456`。按铁律 1 先读 proposal 交接头复述后动手。
