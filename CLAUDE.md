...
- **Phase**:功能 0005「C 端重建④:订单管理」**Phase 3 执行中,前三步(步骤1+2+3)完成 TESTED,进前端页面实现**。分支 `feature/0005-order-manage`(自 main):规划 `947ca28`、快照 `102fa31`、**步骤1 `353f772`**(归属越权 D1)、**步骤2 `a296f2e`**(退款口径 D2)、**步骤3 `8b5d584`**(前端脚手架:4 类型+5 api 函数[historyOrders 用 `pageNum`]+3 路由+3 占位组件)。六决策拍板:D1 归属越权 4 处全修(Service 层)/ D2 退款口径三处统一置 REFUND / D3 历史订单 3 tab+van-list / D4 三页三路由+Menu「我的」入口 / D5 用户中心纯导航壳。验证:后端 verifier 8/8+4/4;步骤3 type-check exit0 + curl(pageNum 返 {total,records})+ preview(3 路由到占位页 + /menu 回归 + 无 error console)。(0004 merge `b08bf8e`、0003 `3365f69`、0002 `df53f0b`、0001 `b02590b` 均已交付。)
- **进行中**:epic「C 端完整重建」—— **0001–0004 已交付,0005(收官)Phase 3 执行中(6 步:步骤1/2/3 done,后端+脚手架就位,余步骤4–6 前端页面)**。跨功能决策(Vant / 整站门槛 / 改造复用 / 认证复用 0001)已落地。
- **下一步**:**0005 Phase 3 步骤4(前端历史订单页 List.vue,D3)**:`van-tabs`(全部/待付款/已取消→status 空/1/6)+ `van-list` `@load` 触底无限滚动(pageNum++、records 累加、finished=len>=total);**切 tab 一次性复位 list/finished/loading/pageNum、首拉只靠 van-list 自动 @load 勿并发双请求、空 tab 立即 finished**(AD1 护栏);卡片(号/时间/金额/状态/菜品缩略)按状态出按钮(再来一单 always / 去支付 status1 带 orderNumber+amount+**orderId** / 催单 status2);点卡片→`/order-detail/:id`。派 subagent 实现(铁律 8)、verifier 跑 preview_network 测试门(tab 过滤/无限滚动不重复/按钮按状态/跳详情;**需 >10 单可能要造单**)。之后步骤5(Detail+成功页接线)→6(Center+Menu入口)。dev server(user-web:5173)+ 新 jar(:8080)在跑;登录门槛页验证用 preview_eval fetch 登录写 `sky_user_token`/`sky_user_info` 捷径。按铁律 1 先读 proposal 交接头复述后动手。
