...
- **Phase**:功能 0005「C 端重建④:订单管理」**Phase 3 执行中,后端两步(步骤1+2)完成 TESTED,进前端**。分支 `feature/0005-order-manage`(自 main):规划 `947ca28`、快照 `102fa31`、**步骤1 code `353f772`**(归属越权 D1)、docs `ac26e33`、**步骤2 code `a296f2e`**(退款口径 D2)。六决策拍板:D1 归属越权 4 处全修(Service 层)/ D2 退款口径三处统一置 REFUND / D3 历史订单 3 tab+van-list / D4 三页三路由+Menu「我的」入口 / D5 用户中心纯导航壳。后端已落:步骤1 新增 user-only `getUserOrderDetail`+3 处补归属+`reminder` `status==2` 守卫(共用 `details()`/`getById` 签名未动,verifier 8/8);步骤2 `rejection`/`cancel` 补 `setPayStatus(REFUND)`+三处统一 `Orders.PAID.equals(payStatus)`(`userCancelById` 不动,verifier 4/4)。(0004 merge `b08bf8e`、0003 `3365f69`、0002 `df53f0b`、0001 `b02590b` 均已交付。)
- **进行中**:epic「C 端完整重建」—— **0001–0004 已交付,0005(收官)Phase 3 执行中(6 步:步骤1、2 done,后端收尾,余纯前端)**。跨功能决策(Vant / 整站门槛 / 改造复用 / 认证复用 0001)已落地。
- **下一步**:**0005 Phase 3 步骤3(前端脚手架:类型+api+路由)**:`types/business.ts` 补 `Order`/`OrderDetail`/`OrderDetailItem`/`PageResult<T>`;`api/order.ts` 补 5 函数(**historyOrders 参数名 `pageNum` 非 `page`**、orderDetail/reminder/repetition/cancel,判成功 `res.code===1`);`router` 加 3 路由(`/order-list`/`/order-detail/:id`/`/user`,懒加载+登录门槛)+ 3 占位组件过 type-check。测试门:`type-check` exit0 + curl 硬验 historyOrders `?pageNum=1&pageSize=10` 返 `{total,records}` 结构。独立无依赖(契约已定死),派 subagent 实现(铁律 8)。之后步骤4(List)→5(Detail+成功页接线)→6(Center+Menu入口)。按铁律 1 先读 proposal 交接头复述后动手。
