...
- **Phase**:功能 0005「C 端重建④:订单管理」**进入 Phase 3(执行),步骤1 完成 TESTED**。分支 `feature/0005-order-manage`(自 main):规划 `947ca28`(Requirement + ADR-0005 五决策 D1–D5 + AD1 双路评审 + 契约校准 + Proposal 6 步)、快照 `102fa31`、**步骤1 code `353f772`**(后端归属越权修复 D1 + AD1 订正)。六决策拍板:D1 归属越权 4 处全修(Service 层)/ D2 退款口径三处统一置 REFUND / D3 历史订单 3 tab+van-list / D4 三页三路由+Menu「我的」入口 / D5 用户中心纯导航壳。步骤1 已落:新增 user-only `getUserOrderDetail`、`userCancelById`/`reminder`/`repetition` 补归属、`reminder` 加 `status==2` 守卫,**共用 `details()` 与 `getById` 签名未动**;verifier 8/8 PASS(4 接口归属隔离 + 催单守卫 + 管理端详情无回归 + 历史订单隔离)。(0004 merge `b08bf8e`、0003 `3365f69`、0002 `df53f0b`、0001 `b02590b` 均已交付。)
- **进行中**:epic「C 端完整重建」—— **0001–0004 已交付,0005(收官)Phase 3 执行中(6 步:步骤1 done)**。跨功能决策(Vant / 整站门槛 / 改造复用 / 认证复用 0001)已落地。
- **下一步**:**0005 Phase 3 步骤2(后端退款口径统一 D2 + AD1 订正)**:管理端 `rejection`/`cancel` 各补 `orders.setPayStatus(Orders.REFUND)`;三处已支付判断统一 `Orders.PAID.equals(payStatus)`(含 `rejection` 既有 `== Orders.PAID`、`cancel` 字面量 `payStatus==1`),`userCancelById` 不动、只补退款状态不碰其余流转。独立 commit,派 subagent 实现(铁律 8)、独立 verifier 跑退款口径测试门(含"未支付不误退"锁**可达路径**=user cancel status1 / admin cancel,**勿用 rejection 测**恒真假绿)。之后步骤3–6(前端脚手架→List→Detail→Center)。按铁律 1 先读 proposal 交接头复述后动手。
