...
## ⭐ 交接头(覆盖式,永远只写"现在")
- **当前**:**Phase 3 步骤1(commit `75ae4bd`)+ 步骤2 后端均已 TESTED**(2026-07-23;步骤2 commit 待提交)。**后端 mock 支付链路已完成。** 步骤1:payment 改内部同步 mock —— `getByNumberAndUserId` 取单(存在+归属)→ 原子 CAS `updateToPaidIfUnpaid`(`WHERE ... pay_status=0`,影响行数 0 拒"该订单已支付"/1 推来单提醒)→ `payment` void、`OrderController` 返 `Result.success()`;删 `paySuccess`/`OrderPaymentVO`/`PayNotifyController`(后者由步骤2 提前,Tech Lead 拍板:paySuccess 唯一调用者)/`userMapper` 字段;去 openid/微信 `pay`;4 门全 PASS。步骤2:删 `WeChatPayUtil`/`WeChatProperties`/`sky.wechat` 配置(2 文件)/两处 pom `wechatpay-apache-httpclient` 依赖/`weChatPayUtil` 字段+import;3 处 refund 逐处枚举换 mock log(`userCancelById` 保留 `Orders.REFUND` 口径,`rejection`/`cancel` 不置 REFUND 的既有不一致留 0005)。测试门全 PASS:①`mvn` EXIT=0 + jar 干净启动(证 `@ConfigurationProperties` 解绑净)②退款 mock(取消 id=6 → `code:1` / DB `status=6 已取消` / 日志"模拟退款")③grep 归零(7 token 在 `sky-take-out` 下 0 命中)。
- **下一步**:**提交步骤2 commit** → 进 **步骤3(前端:支付页 + 成功页改造 + 接线)**:`api/order.ts` 补 `payment`;新增 `Order/Pay.vue`(金额 + 支付方式单选 + 确认支付 → `payment` → `code===1` 跳成功页并 query 透传 `orderNumber`(+`amount`)/ 否则错误提示不跳;按钮 `submitting` 守卫防双击);`Order/Created.vue` 改造成成功页(下单成功 + 返回菜单 + 查看订单禁用/占位标注 0005,读 query `orderNumber` 备 0005);路由新增支付页 + 成功页(登录门槛);`Order/Confirm.vue` 下单成功 push 支付页(带 `orderNumber`/`amount`)。测试门(端到端 preview + `preview_network`,verifier):落点改造 / 确认支付→成功页 / 成功页行为 / 端到端 / 负例 `code!==1` 不跳 / 回归 `type-check` + `/menu` + `/order-confirm`。
...
- [x] **步骤2(后端)· TESTED(2026-07-23)**:去微信支付删干净(D4)—— 删 `PayNotifyController`(步骤1 已删)/`WeChatPayUtil`/`WeChatProperties`/`sky.wechat` 配置 + pom 依赖 + 3 处 refund 换 mock  [依赖: 1]
      **实测**:①编译 `mvn` EXIT=0 + jar 干净启动 ②取消 id=6 已支付单 → `code:1`/DB `status=6 已取消`/日志"模拟退款" ③grep 7 token 在 `sky-take-out` 下 0 命中(`Orders.REFUND` 状态枚举合法保留、不计)。3 处 refund 用变量:`userCancelById`=`orderDB`(保留 `setPayStatus(REFUND)`)、`rejection`=`ordersDB`、`cancel`=`orderDB`(后二者既有不置 REFUND 口径不动,留 0005)。
