...
- [x] **步骤3(前端)· TESTED(2026-07-23)**:支付页 + 成功页改造 + 接线  [依赖: 1(契约锁定,可并行开发;联调需步骤1 jar)]
      **实测**(端到端 preview + `preview_network`,独立 verifier):①去支付 → `/order-pay`(非占位),金额 ¥11 与结算/query `orderAmount` 一致 ②确认支付 `PUT /user/order/payment` → `code:1` → `/order-created`"下单成功",DB `status=2/pay_status=1`(单号 1784820516284)③返回菜单跳 `/menu`、查看订单 `disabled` 含"0005 接管"不跳、query 带 orderNumber ⑤负例:已支付单 → `code:0`"该订单已支付"、URL 停 `/order-pay`、toast 显示不跳 ⑥回归:`/menu`+`/order-confirm` 正常渲染无致命报错;`type-check` EXIT=0。落地细节:金额键 `orderAmount`(非 `amount`),`request.ts` 返回整 `Result`、判 `res.code===1`;成功页原地复用 `Created.vue`(路由 `/order-created` 语义转为支付成功页)。
