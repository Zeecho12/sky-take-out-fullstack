...
### 做什么 (In Scope)
- **后端 payment 重写(D1+D2+D3;CAS 见 AD1)**:`OrderServiceImpl.payment()` 去 `weChatPayUtil.pay` + 去 openid 读取,改为**内部同步**——取单校验(存在 + 归属)后用**原子条件更新 CAS**(`OrderMapper.updateToPaidIfUnpaid`,`WHERE ... AND pay_status=0`)置「已支付 / 待接单」,按影响行数判成败(0 → 拒"该订单已支付";1 → 推来单提醒);**删 `paySuccess`**(推送并入 payment)+ 删 `userMapper` 字段 + `JSONObject` import;`payment` 返回值简化(去微信预支付 5 字段,响应只表"成功");随之调整 `OrderService.payment` / `OrderController.payment` 签名 + 删 `OrderPaymentVO`。
- **后端去微信支付删干净(D4)**:删 `controller/notify/PayNotifyController.java`、`utils/WeChatPayUtil.java`、`properties/WeChatProperties.java`;删 `application.yml` / `application-dev.yml` 的 `sky.wechat.*` 配置块 + **两处 pom 的 `wechatpay-apache-httpclient` 依赖**(grep 确认仅上述引用);删 `OrderServiceImpl` 的 `weChatPayUtil` 字段 + import。
...
- **支付超时倒计时业务 / 分布式幂等**:reference 有 15 分钟支付倒计时 + 超时取消,0004 **不做倒计时业务**(最低可用)。重复支付用**数据库层 CAS**(`WHERE pay_status=0`,评审 AD1 采纳)做单节点原子幂等——这是**做**了的;分布式去重键 / 完整支付状态机留将来记档。
...
- [ ] **(负例 · 重复支付 · 原子幂等)** 对已支付订单再打一次 `PUT /order/payment` → 后端**拒**(`OrderBusinessException`"该订单已支付"),订单状态不变、**不重复推来单提醒**(靠 CAS `WHERE pay_status=0` 影响行数=0 判定,非 check-then-act 弱防呆;评审 AD1)。curl 硬验。
...
- [ ] **成功页**:显示"下单成功"、"返回菜单"可点(跳 `/menu`);**"查看订单"按钮禁用 / 占位**(标注 0005),点击不跳转 / 无报错。成功页 URL 的 query **带 `orderNumber`**(支付页跳转时透传,给 0005「查看订单」留钩子;评审 AD1 外审#1)。
