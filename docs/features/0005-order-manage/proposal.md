...
## ⭐ 交接头(覆盖式,永远只写"现在")
- **当前**:**Phase 3 步骤1(后端订单归属越权修复 D1 + AD1 订正)完成 TESTED**,code commit `353f772`(独立,仅 3 后端文件)。改动:新增 user-only `OrderService.getUserOrderDetail(id)`(getById→null/归属→装配 OrderVO),`user/OrderController.details` 改调它;`userCancelById`/`reminder` 补 Service 层归属校验;`repetition` 先 `getById` 取单再校验归属;`reminder` 加 `status==2` 业务守卫;**共用的 `details()`、`OrderMapper.getById` 签名均未动**(管理端 `GET /admin/order/details/{id}` 仍共用 `details()`)。测试门(独立 verifier,curl+DB,多用户数据隔离)8/8 通过:4 接口归属隔离 + 催单状态守卫 + 管理端详情无回归 + 历史订单隔离。规划提交 `947ca28`(Requirement+ADR五决策+AD1+契约校准+本 Proposal)、快照 `102fa31` 在前。
- **下一步**:**Phase 3 步骤2(后端退款口径统一 D2 + AD1 订正)**(派 subagent)。`rejection`/`cancel` 各补 `orders.setPayStatus(Orders.REFUND)`;三处已支付判断统一 `Orders.PAID.equals(payStatus)`(含 `rejection` 既有 `== Orders.PAID`、`cancel` 字面量 `payStatus==1`),`userCancelById` 不动。独立 commit,verifier 跑退款口径测试门(含"未支付不误退"锁**可达路径**=user cancel status1 / admin cancel,**勿用 rejection 测**)。之后步骤3–6(前端)。**未做**:步骤2–6。
- **别碰**:`reference/`(只读);管理端**除** `rejection`/`cancel` 的 `payStatus=REFUND` + `.equals()` 改动(D2 步骤2)外一律不动;共用的 `details()` 与 `OrderMapper.getById` 签名(步骤1 已定,后续也别碰);0002–0004 已交付代码**除** Menu 顶栏加「我的」入口 + `Confirm.vue`/`Pay.vue`/`Created.vue` 透传 `orderId` + `Created.vue` 查看订单接线(步骤5/6)**外**不动。
- **环境现状**:新 jar(PID 28664)在 :8080 运行(含步骤1 改动);`sky-redis` up;MySQL 5.7 本机 `localhost:3306/sky_take_out` root/123456(连库加 `--ssl-mode=DISABLED`,client `D:\HSPJAVA\mysql-5.7.19-winx64\bin\mysql.exe`)。**步骤2 改后端前先停旧 jar(找 :8080 监听进程)→ 重建(IDE 内置 Maven `mvn -DskipTests package`,`JAVA_HOME=D:\Program\hspjdk17`)→ 起新 jar**。⚠️ 环境扛不过 Claude Code 进程重启,新窗口先核 `java`/8080/6379/`docker ps`。
...
- [ ] **步骤2(后端)· 退款状态口径统一(D2 + AD1 订正)**,独立 commit  [依赖: 无]  —— TODO
...
- [ ] **步骤3(前端)· 脚手架:类型 + api + 路由**  [依赖: 无(契约已定死)]  —— TODO
