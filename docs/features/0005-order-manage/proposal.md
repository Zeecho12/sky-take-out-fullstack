...
## ⭐ 交接头(覆盖式,永远只写"现在")
- **当前**:**Phase 3 前四步完成 TESTED**(步骤1 归属越权 D1 + 步骤2 退款口径 D2 + 步骤3 脚手架 + 步骤4 历史订单页 List.vue)。commit:`353f772`(D1)、`a296f2e`(D2)、`8b5d584`(脚手架)、`7c961f9`(List.vue,含 van-list 切 tab 缺陷修复)。验证:后端 verifier 8/8+4/4;步骤3 type-check+curl+preview;步骤4 preview mobile 重验全 PASS(tab 过滤各切 tab 恰 1 请求且自动加载、无限滚动首屏单请求+触底无重复+finished 停、按钮按状态、点卡片跳详情)。**后端+脚手架+历史订单页就位,进详情页。**
- **下一步**:**Phase 3 步骤5(前端订单详情页 Detail.vue + 动作 + 成功页接线,D4)**(派 subagent,依赖步骤3 已满足)。`views/Order/Detail.vue`(现为占位)实现:`GET orderDetail/{id}`(`route.params.id`)全字段 + 明细列表;按状态出操作(取消 status1|2→`showConfirmDialog`→`cancel`→刷新为已取消 / 立即支付 status1→`/order-pay` 带 orderNumber+orderAmount+orderId / 催单 status2→`reminder`→toast / 再来一单 always→`showConfirmDialog`→`repetition` 合并→`cart.refresh`→`/menu`,AD1 Q1);**`Created.vue`「查看订单」去 disabled + `@click`→`/order-detail/${orderId}`,缺 orderId 兜底(禁用/退化跳 `/order-list`,不跳 `/order-detail/undefined`)**;`Confirm.vue`/`Pay.vue` query **每跳补透传 `orderId`**(源自 submit `OrderSubmitVO.id`)。可复用 List.vue 的动作写法(cancel/reminder/repetition/goPay)。测试门交 verifier(preview mobile:详情字段、取消/支付/催单/再来一单合并、成功页接线闭环、orderId 兜底负例)。之后步骤6(Center+Menu入口)。**未做**:步骤5–6。
- **别碰**:`reference/`(只读);后端订单代码(步骤1/2 已定稿,除非发现回归否则不再动);共用的 `details()`/`OrderMapper.getById` 签名;步骤3 已建的类型/api/路由 + 步骤4 的 `List.vue`(步骤5 复用其动作写法,别改 List.vue);0002–0004 已交付代码**除** Menu 顶栏「我的」入口(步骤6)+ `Confirm.vue`/`Pay.vue`/`Created.vue` 透传 `orderId` + `Created.vue` 查看订单接线(步骤5)**外**不动。
- **环境现状**:新 jar(PID 3188)在 :8080 运行(含步骤1+2);`sky-redis` up(店铺营业中);**C 端 dev server `user-web` 在 :5173**(serverId 每次 `preview_list` 查;**验证前先 `preview_resize` mobile**——预览初始 `innerHeight=0` 会让 van-list 不触发 @load);MySQL 5.7 本机 `localhost:3306/sky_take_out` root/123456(`--ssl-mode=DISABLED`,client `D:\HSPJAVA\mysql-5.7.19-winx64\bin\mysql.exe`)。⚠️ 环境扛不过 Claude Code 进程重启,新窗口先核 `java`/8080/6379/`docker ps` + preview_list。**登录注入捷径**(路由受登录门槛):preview_eval `fetch('/api/user/user/login',{method:'POST',...})` 拿 token → `localStorage.setItem('sky_user_token',token)`+`sky_user_info`(`{id,username}`)→ reload。账号 `s7v_2268`/`123456`(id=8)。测试数据:甲(id=8)现有 **16 单**(status1×3=id13/14/15、status2×2=id16/17、其余 status6),够步骤5 取详情/各状态动作(`select id,status,pay_status from orders where user_id=8`)。
