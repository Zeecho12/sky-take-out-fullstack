...
## ⭐ 交接头(覆盖式,永远只写"现在")
- **当前**:**Phase 3 后端两步(步骤1 归属越权 D1 + 步骤2 退款口径 D2)均完成 TESTED**。commit:步骤1 `353f772`(归属:新增 user-only `getUserOrderDetail`、`userCancelById`/`reminder`/`repetition` 补归属、`reminder` 加 `status==2` 守卫,共用 `details()`/`getById` 签名未动)、步骤2 `a296f2e`(`rejection`/`cancel` 补 `setPayStatus(REFUND)`、三处已支付判断统一 `Orders.PAID.equals(payStatus)`,`userCancelById` 不动)。docs `ac26e33`(步骤1)在前。验证:步骤1 verifier 8/8(归属隔离+催单守卫+管理端详情无回归+历史隔离)、步骤2 verifier 4/4(三处退款口径 pay2+未支付不误退)。**后端改动全部收尾,剩下纯前端。**
- **下一步**:**Phase 3 步骤3(前端脚手架:类型 + api + 路由)**(派 subagent)。`types/business.ts` 补 `Order`/`OrderDetail`/`OrderDetailItem`/`PageResult<T>`;`api/order.ts` 补 5 函数(**historyOrders 参数名 `pageNum` 非 `page`**、orderDetail/reminder/repetition/cancel);`router` 加 3 路由(`/order-list`/`/order-detail/:id`/`/user`,登录门槛)+ 3 占位组件过 type-check。测试门:`type-check` exit0 + curl 硬验 historyOrders 用 `?pageNum=1&pageSize=10` 返 `{total,records}` 结构。之后步骤4(List)→5(Detail+接线)→6(Center+入口)。**未做**:步骤3–6。
- **别碰**:`reference/`(只读);后端订单代码(步骤1/2 已定稿,除非发现回归否则不再动);共用的 `details()`/`OrderMapper.getById` 签名;0002–0004 已交付代码**除** Menu 顶栏「我的」入口 + `Confirm.vue`/`Pay.vue`/`Created.vue` 透传 `orderId` + `Created.vue` 查看订单接线(步骤5/6)**外**不动。
- **环境现状**:新 jar(PID 3188)在 :8080 运行(含步骤1+2 改动);`sky-redis` up;MySQL 5.7 本机 `localhost:3306/sky_take_out` root/123456(连库加 `--ssl-mode=DISABLED`,client `D:\HSPJAVA\mysql-5.7.19-winx64\bin\mysql.exe`)。前端步骤 3–6 起 C 端:`preview_start` name `user-web` 或 `npm --prefix project-sky-user-vue3 run dev`(:5173);联调需后端在跑 + `PUT /admin/shop/1`(Bearer,dish/setmeal 依赖 Redis 店铺状态)。⚠️ 环境扛不过 Claude Code 进程重启,新窗口先核 `java`/8080/6379/`docker ps`。测试数据:step1/2 verifier 已消耗订单 7/9/10/11/12(置 status6),甲(id=8)现有订单见 DB。
