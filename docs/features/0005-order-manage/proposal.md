...
## ⭐ 交接头(覆盖式,永远只写"现在")
- **当前**:**Phase 3 前三步完成 TESTED**(步骤1 归属越权 D1 + 步骤2 退款口径 D2 + 步骤3 前端脚手架)。commit:步骤1 `353f772`、步骤2 `a296f2e`、步骤3 `8b5d584`(前端脚手架:`types/business.ts` 补 4 类型 + `api/order.ts` 补 5 函数[historyOrders 用 `pageNum`] + `router` 加 3 路由 + 3 占位组件)。验证:后端两步 verifier 8/8 + 4/4;步骤3 type-check exit0 + verifier curl(historyOrders?pageNum=1 返 `{total,records}`、5 接口连通)+ preview(3 路由解析到占位页 + /menu 回归 + 无 error console)。**后端收尾,前端脚手架就位,进页面实现。**
- **下一步**:**Phase 3 步骤4(前端历史订单页 List.vue,D3)**(派 subagent,依赖步骤3 已满足)。`views/Order/List.vue`(现为占位)实现:`van-tabs`(全部/待付款/已取消 → status 空/1/6)+ `van-list` `@load` 触底(`pageNum++`、records 累加、`finished=len>=total`);**切 tab 一次性复位 `list=[]`/`finished=false`/`loading=false`/`pageNum=1`,首拉只靠 van-list 挂载自动 `@load`(勿手动+自动并发→双请求),空 tab 立即 finished**(AD1 护栏);卡片(号/时间/金额/状态/菜品缩略=占位图)按状态出按钮(再来一单 always / 去支付 status1→`/order-pay` 带 orderNumber/orderAmount/**orderId** / 催单 status2);点卡片→`/order-detail/:id`;空态。测试门交 verifier(preview+preview_network:tab 过滤、无限滚动不重复、按钮按状态、点卡片跳详情)。之后步骤5(Detail+成功页接线)→6(Center+Menu入口)。**未做**:步骤4–6。
- **别碰**:`reference/`(只读);后端订单代码(步骤1/2 已定稿,除非发现回归否则不再动);共用的 `details()`/`OrderMapper.getById` 签名;步骤3 已建的类型/api/路由(步骤4 直接用,别重定义);0002–0004 已交付代码**除** Menu 顶栏「我的」入口 + `Confirm.vue`/`Pay.vue`/`Created.vue` 透传 `orderId` + `Created.vue` 查看订单接线(步骤5/6)**外**不动。
- **环境现状**:新 jar(PID 3188)在 :8080 运行(含步骤1+2);`sky-redis` up(店铺营业中);**C 端 dev server 已起**(`preview_start` name `user-web`,:5173,serverId 用 preview_list 查);MySQL 5.7 本机 `localhost:3306/sky_take_out` root/123456(`--ssl-mode=DISABLED`,client `D:\HSPJAVA\mysql-5.7.19-winx64\bin\mysql.exe`)。⚠️ 环境扛不过 Claude Code 进程重启,新窗口先核 `java`/8080/6379/`docker ps` + preview_list。**登录注入捷径**(路由受登录门槛):preview_eval `fetch('/api/user/user/login',{method:'POST',...})` 拿 token → `localStorage.setItem('sky_user_token',token)`+`sky_user_info`(`{id,username}`)→ reload(store 从这两个 key 初始化)。账号 `s7v_2268`/`123456`(id=8)。测试数据:step1/2/3 verifier 已消耗订单 7/8/9/10/11/12(多置 status6/退款),甲现有订单见 DB(`select id,status,pay_status from orders where user_id=8`)——**步骤4 无限滚动需 >10 单,verifier 可能要造单**。
...
- [ ] **步骤4(前端)· 历史订单页 List.vue(D3)**  [依赖: 3]  —— TODO
...
- [ ] **步骤5(前端)· 订单详情页 Detail.vue + 动作 + 成功页接线(D4)**  [依赖: 3]  —— TODO
