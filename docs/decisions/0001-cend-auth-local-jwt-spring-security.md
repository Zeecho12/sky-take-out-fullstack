...
- `divedeep`:**JWT `OncePerRequestFilter`** 如何填 `SecurityContext`,以及为何不做成
  `@Component`(落 AD2)。
- **✅ 已写(2026-07-23,含金量:高)**:上述三条候选已**合并**成一篇 `divedeep` —— 见
  `docs/divedeep/JWT + Spring Security 认证链路精读:过滤器链、Token 校验与 ThreadLocal 当前用户注入.md`
  (一条链讲透:请求撞过滤器链 → `JwtAuthenticationFilter` 验票 → 双写 `SecurityContextHolder` + `BaseContext`(ThreadLocal)→ `hasRole` 授权 → 业务 `getCurrentId()` 取用;"认证时序"作签票上游一并覆盖)。
